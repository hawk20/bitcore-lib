...
var ufocore = require('..');
var ufocoinjs = require('ufocoinjs-lib');
...
async.series([
  function(next) {
    ...
    for (var i = 0; i < 100; i++) {
      ...
      // uint64le
      var br = new ufocore.encoding.BufferWriter();
      var num = Math.round(Math.random() * 10000000000000);
      br.writeUInt64LEBN(new ufocore.crypto.BN(num));
      buffers.push(br.toBuffer());

      // hashes
      var data = ufocore.crypto.Hash.sha256sha256(new Buffer(32));
    ...
    function readUInt64LEBN() {
      ...
      var buf = buffers[c];
      var br = new ufocore.encoding.BufferReader(buf);
  ...
  function(next) {
    ...
    function ufocoreTest() {
      block1 = ufocore.Block.fromString(blockData);
    }

    function ufocoinJSTest() {
      block2 = ufocoinjs.Block.fromHex(blockData);
    }
    ...
    var suite = new benchmark.Suite();
    suite.add('ufocore', ufocoreTest, {maxTime: maxTime});
    suite.add('ufocoinjs', ufocoinJSTest, {maxTime: maxTime});
